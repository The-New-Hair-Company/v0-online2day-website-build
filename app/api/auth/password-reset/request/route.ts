...
export async function POST(request: Request) {
  ...
  const rate = await enforceAuthRateLimit(request, 'password-reset-request', 3)
  if (!rate.ok) return authRateLimitJson(rate, 3, 'Too many reset requests. Please wait before trying again.')
