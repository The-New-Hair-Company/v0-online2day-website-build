...
function Sidebar() {
  const navItems: Array<{ label: string; icon: IconName; active?: boolean; badge?: string }> = [
    { label: 'Overview', icon: 'grid' },
    { label: 'Leads', icon: 'users', active: true },
    { label: 'Videos', icon: 'video' },
    { label: 'Emails', icon: 'mail' },
    { label: 'Messages', icon: 'message', badge: '4' },
    { label: 'Site Requests', icon: 'request' },
  ...
  return (
    <aside className={styles.sidebar}>
      ...
      <nav className={styles.nav} aria-label="CRM navigation">
        ...
        {navItems.map((item) => (
          <a key={item.label} className={cx(styles.navItem, item.active && styles.navItemActive)} href="#">
            <Icon name={item.icon} />
